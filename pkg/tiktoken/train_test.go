...
// TestTrainBasic tests the training of a basic text.
func TestTrainBasic(t *testing.T) {
	debugTrain = true
	txt := "i'm blue dabadee dabadam"
	vocab := Train(txt, 264, CL100KBaseSplitPattern)
	ReadVocab(vocab, os.Stdout)
	t.Logf("vocab: %v", vocab)
}

// TestTrainBasicWithNums tests the training of a basic text with numbers.
...
// TestTrainPlease tests the training of a text with a lot of repeating characters.
func TestTrainPlease(t *testing.T) {
	...
	if vocab["ee"] != 256 || vocab["eeee"] != 257 || vocab["ug"] != 258 {
		t.Errorf(
			`vocab["ee"]=%v, vocab["eeee"]=%v, vocab["ug"]=%v`,
			vocab["ee"],
			vocab["eeee"],
			vocab["ug"],
		)
...
// TestTrainArtificialForBlogPost tests the training of an artificial text for a blog post.
func TestTrainArtificialForBlogPost(t *testing.T) {
	txt := "abcdede zdede tXOXO vXO"
	vocab := Train(txt, 260, CL100KBaseSplitPattern)
	if vocab["de"] != 256 || vocab["XO"] != 257 || vocab["dede"] != 258 {
		t.Errorf(
			`vocab["de"]=%v, vocab["XO"]=%v, vocab["dede"]=%v`,
			vocab["ee"],
			vocab["eeee"],
			vocab["ug"],
		)
